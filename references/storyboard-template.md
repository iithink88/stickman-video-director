...
Create one natural **Chinese (Mandarin)** narration of **240–300 characters** for approximately 55–65 seconds of speech (≈4.5 characters per second).
...
- Prefer clear spoken Mandarin to literal translation.
...
Use the user's language for planning explanations. **Write the voiceover directly in Chinese** — no English draft, no reference translation column.
...
4. Narrator identity, speaking pace, Chinese character count, and estimated duration
...
Default the narrator only after required setup is complete: a bright, energetic adult female voice speaking **standard Mandarin** (本机对应 `edge-tts zh-CN-XiaoxiaoNeural`，或 apiz `minimax/t2a` 的 `female-yujie`). Infer tone and accent colors from the source when the user did not specify them.
...
| 时间 | 叙事作用 | 火柴人画面 | 运动·运镜·转场 | 中文旁白 | 字数 | BGM / 音效 |
|---|---|---|---|---|---|---|

Give each row a different narrative job. Allocate approximately **40–50 Chinese characters** per row while keeping sentence boundaries natural.
...
Default the generated video to no visible words, letters, numbers, captions, subtitles, interface copy, or technical annotations. Make message bubbles, content cards, meters, clocks, and notifications icon-only. After the storyboard, optionally list concise **2–5 character Chinese** overlays for post-production, including their target clips and safe placement; never carry those overlays into the video-generation prompts.
...
- Chinese narration is 240–300 characters and approximately 55–65 seconds.
